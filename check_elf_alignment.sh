...
# Enhanced Summary Section - FIXED LOGIC
if [ ${#critical_unaligned_libs[@]} -gt 0 ]; then
  # TRUE FAILURE CASE - Critical libraries are unaligned
  summary_lines=(
    "${ALERT} CRITICAL: Unaligned libraries require immediate action!"
    ""
    "Analysis Results:"
    ...
    "  • CRITICAL failures: ${#critical_unaligned_libs[@]} (MUST FIX)"
  )
  ...
  # Fix instructions for critical failures
  print_section "$TOOLS" "How to Fix Critical Issues" "Urgent fixes required for Google Play compliance"
  ...
  print_subsection "Step 3: Rebuild and Verify"
  print_status "processing" "Clean Build" "Run ./gradlew clean"
  print_status "processing" "Rebuild Project" "Run ./gradlew assembleRelease"
  print_status "processing" "Re-run This Script" "Verify all critical libraries are now aligned"

  print_summary_box "URGENT ACTION REQUIRED" "error" \
...
elif [ ${#non_critical_unaligned_libs[@]} -gt 0 ]; then
  # GOOGLE PLAY COMPLIANT - Only non-critical warnings
  summary_lines=(
    "${PARTY} Google Play Compliance: PASSED!"
    ""
    "Analysis Results:"
    "  • Total libraries scanned: $total_libs"
    "  • Critical architectures (64-bit): $critical_libs - ALL ALIGNED ${CHECK_MARK}"
    "  • Libraries aligned: ${#aligned_libs[@]}"
    "  • Non-critical warnings: ${#non_critical_unaligned_libs[@]} (ARMv7 - optional to fix)"
    ""
    "Google Play Compliance: PASSED ${CHECK_MARK}"
    "Ready for November 1st, 2025 deadline!"
    ""
    "Note: Only 32-bit ARMv7 warnings remain (not required by Google Play)"
  )

  print_summary_box "GOOGLE PLAY COMPLIANT" "success" "${summary_lines[@]}"

  print_subsection "Non-Critical Libraries (Optional Improvements)"
  for lib in "${non_critical_unaligned_libs[@]}"; do
    arch_type="32-bit"
    [[ "${lib}" == *"armeabi-v7a"* ]] && arch_type="ARMv7"
    [[ "${lib}" == *"x86"* ]] && arch_type="x86"
    print_status "warning" "$(basename "$lib")" "$arch_type architecture - Optional to fix"
  done

  print_section "$ROCKET" "Optional Improvements" "Consider these enhancements for complete compatibility"

  print_status "success" "Google Play Compliance" "All critical 64-bit libraries properly aligned"
  print_status "info" "ARMv7 Optimization" "Consider removing 32-bit support for smaller APK size"
  ...
  print_summary_box "CONGRATULATIONS!" "success" \
    "${PARTY} Your app is Google Play compliant!" \
    "" \
    "${CHECK_MARK} All critical 64-bit libraries meet requirements" \
    "${CHECK_MARK} Ready for devices with 16KB page sizes" \
    "${CHECK_MARK} Compliant with November 1st, 2025 deadline" \
    "" \
    "Optional: Fix ARMv7 warnings for complete compatibility" \
    "Your app can be deployed to Google Play Store now!"

  cleanup_trap 0

else
  # PERFECT SUCCESS - All libraries aligned
  summary_lines=(
    "${PARTY} Perfect! All native libraries are properly aligned!"
    ""
    "Analysis Results:"
    "  • Total libraries scanned: $total_libs"
  ...
  print_summary_box "PERFECT COMPATIBILITY" "success" "${summary_lines[@]}"
  ...
  print_summary_box "CONGRATULATIONS!" "success" \
    "${PARTY} Your app has perfect 16KB compatibility!" \
    "" \
    "${CHECK_MARK} All native libraries meet Google Play requirements" \
    "${CHECK_MARK} Ready for devices with 16KB page sizes" \
    "${CHECK_MARK} Compliant with November 1st, 2025 deadline" \
    "" \
    "Your app is fully optimized and ready for deployment!"
